...
public class YoloxDogVideoDetector {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String outputVideo = "output_detected.mp4";
        ...
        if (!cap.isOpened()) {
            throw new RuntimeException("Cannot open video: " + videoPath);
        }
        ...
        while (cap.read(frame)) {
            int percent = (int) ((currentFrame / totalFrames) * 100);
            System.out.print(
                "\rProcessing frame " + currentFrame++ + " / " + (int) totalFrames + " (" + percent + "%)");
        ...
        session.close();
        env.close();
        System.out.println("Video processed: " + outputVideo);
    }

    // Helper class to return preprocessed data and offsets
    static class PreprocessResult {
        float[] data;
        float scale, xOffset, yOffset;
        int origW, origH;
        BufferedImage letterboxed;
    }

    // Preprocess a single frame (similar to image version)
    static PreprocessResult preprocess(BufferedImage img) throws Exception {
    ...
    // Detection class & NMS same as image version
    static class Detection {
        float x1, y1, x2, y2, conf;

        Detection(float x1, float y1, float x2, float y2, float conf) {
            this.x1 = x1;
            this.y1 = y1;
            this.x2 = x2;
            this.y2 = y2;
    ...
    static float iou(Detection a, Detection b) {
        float xx1 = Math.max(a.x1, b.x1), yy1 = Math.max(a.y1, b.y1), xx2 = Math.min(a.x2, b.x2), yy2 =
            Math.min(a.y2, b.y2);
    ...
    // Decode + NMS + draw (same as image version)
        ...
        for (int s = 0; s < 3; s++) {
            ...
            for (int gy = 0; gy < gh; gy++) {
                for (int gx = 0; gx < gw; gx++) {
                    ...
                    float predX = (tx + gx) * stride, predY = (ty + gy) * stride, predW = (float) Math.exp(tw) * stride,
                        predH = (float) Math.exp(th) * stride;
